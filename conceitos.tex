...
A função de similaridade utilizada pelo algoritmo de avaliação de currículos do SILQ é a \textit{n-gram}, com valor de \textit{n} igual a 3 (sendo chamado, neste caso, de \textit{trigram}) \cite{Silq1}. Um \textit{n-gram} é uma sequência de $n$ caracteres contíguos extraídos de um texto qualquer. Por exemplo, a palavra ``Revista" contém o trigrama \textit{``Rev"}. O conjunto de todos os trigramas possíveis para este mesmo exemplo é $\{\_\_R,\_Re,Rev,evi,vis,ist,sta,ta\_\}$ (o início e final da palavra são modelados com ``espaços" por certas funções para explicitamente indicar o início e o final da sequência). A função de similaridade \textit{n-grams} recebe como parâmetro duas sequências de caracteres, calcula os conjuntos de n-gramas de ambas e retorna a similaridade dos parâmetros como sendo a fração de elementos em comum dos dois conjuntos \cite{Broder97}.
