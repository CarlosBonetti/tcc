...
\section{Sistemas de Recuperação de Informação e métricas de avaliação}
...
A média de rank recíproco (\textit{MRR - Mean Reciprocral Rank}), é uma medida estatística utilizada na avaliação de processos que produzem uma lista de resultados a uma \textit{query} ordenados por probabilidade de corretude \cite{Liu2009}.
...
As \textit{queries} do conjunto de testes são dadas ao sistema e os resultado retornados comparados com aqueles informados pelo especialista, produzindo uma série de medidas que podem ser utilizadas para a avaliação do sistema \cite{Witten2005}.
