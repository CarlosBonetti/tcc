...
A qualidade da produção intelectual de grupos de pesquisa é classificada pela CAPES (Coordenação de Aperfeiçoamento de Pessoal de Nível Superior) através de um conjunto de procedimentos denominado Qualis. O Qualis mede a qualidade de artigos e outras produções a partir da análise da qualidade dos veículos onde eles foram divulgados, ou seja, periódicos e eventos científicos. Esta classificação é realizada em um processo anual de atualização, sendo os veículos enquadrados nos estratos indicativos de qualidade A1, A2, B1, B2, B3, B4, B5 e C, do maior para o menor peso, para cada área do conhecimento \cite{CAPES-Classificacao}.
...
Outra questão relacionada à primeira versão do SILQ é a falta de alguns recursos que poderiam facilitar o processo de avaliação e acompanhamento de currículos, como gráficos de classificações dentro de grupos de pesquisa. Uma característica importante também seria a capacidade de integração com outros sistemas, facilitando o reuso do serviço disponibilizado pelo SILQ em trabalhos futuros.

Mais um ponto de melhoria seria a avaliação experimental do algoritmo de classificação do SILQ para o estabelecimento de uma medida do grau de precisão da ferramenta. A partir disto, outras técnicas podem ser propostas e comparadas com as anteriores a fim de provar se elas resultaram em ganho de precisão ou não.
...
Também propõe-se a implementar controles que permitam ao usuário fornecer \textit{feedback} de relevância sobre os resultados retornados pelo sistema, ou seja, permitir ao usuário sugerir o melhor \textit{matching} caso o sistema não encontre um candidato satisfatório, e assim alimentar o algoritmo de classificação com o objetivo de melhorar as respostas subsequentes. Por fim, pretende-se realizar uma avaliação experimental do grau de exatidão do algoritmo de classificação antes e depois das mudanças realizadas.

A hipótese de pesquisa a ser avaliada a partir desta ideia, portanto, é avaliar se o feeback explícito de usuários pode aumentar a acurácia do SILQ, um sistema de avaliação baseada em similaridade textual.

\section{Objetivos}

O objetivo geral deste trabalho é analisar o impacto que o uso de \textit{feedback} de relevância tem na precisão dos resultados de avaliações realizadas pelo SILQ, e uma reestruturação arquitetural da ferramenta, incluindo a criação de API de integração com outros sistemas e a atualização da base de dados conforme as novas classificações Qualis.

%O objetivo geral deste trabalho é a reestruturação arquitetural do Sistema de Integração Lattes-Qualis (SILQ), incluindo a criação de API de integração com outros sistemas e a atualização da base de dados conforme novas classificações Qualis, e uma análise do uso de feedback de relevância e seu impacto na precisão dos resultados de avaliações de publicações, baseadas em similaridade textual.
...
\begin{itemize}
    ...
    %\item \afazer{Criação de uma documentação da nova API REST com o objetivo de auxiliar outros serviços a se integrarem com o SILQ};
    ...
    \item Propor algoritmos de avaliação baseados em \textit{feedback} de relevância e verificar se a taxa de exatidão do sistema foi melhorada com tal ação.
\end{itemize}
